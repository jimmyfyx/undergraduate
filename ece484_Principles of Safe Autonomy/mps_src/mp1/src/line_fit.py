...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		LU_pixel_1 = (leftx_current - margin, y_0 - (window + 1) * window_height)  # Left window
		RD_pixel_1 = (leftx_current + margin  , y_0 - (window) * window_height)
		LU_pixel_2 = (rightx_current - margin , y_0 - (window + 1) * window_height)  # Right window
		RD_pixel_2 = (rightx_current + margin  , y_0 -(window) * window_height)

		# Draw the windows on the visualization image using cv2.rectangle()
		out_img = cv2.rectangle(out_img, LU_pixel_1 , RD_pixel_1 , (0,255,255), 5)
		out_img = cv2.rectangle(out_img, LU_pixel_2 , RD_pixel_2 , (0,255,255), 5)

		# Identify the nonzero pixels in x and y within the window
		# Non-zero coordinates in the left window
		x_min = LU_pixel_1[1]
		x_max = RD_pixel_1[1] 
		y_min = LU_pixel_1[0]
		y_max = RD_pixel_1[0]
		ranges= (array[:, 0] >= x_min) & (array[:, 0] <= x_max) & (array[:, 1] >= y_min) & (array[:, 1] <= y_max)
		left_coords = array[ranges]

		# Non-zero coordinates in the right window
		x_min = LU_pixel_2[1]
		x_max = RD_pixel_2[1] 
		y_min = LU_pixel_2[0]
		y_max = RD_pixel_2[0]
		ranges = (array[:, 0] >= x_min) & (array[:, 0] <= x_max) & (array[:, 1] >= y_min) & (array[:, 1] <= y_max)
		right_coords = array[ranges]

		# Append the coorindates to the lists
		left_lane_inds.append(left_coords)
		right_lane_inds.append(right_coords)

		# If you found > minpix pixels, recenter next window on their mean position
		if(left_coords.shape[0] > minpix):
			leftx_current = int(np.mean(left_coords[:, 1]))
		if(right_coords.shape[0] > minpix):
			rightx_current = int(np.mean(right_coords[:, 1]))
		
	# Concatenate the arrays of coordinates
