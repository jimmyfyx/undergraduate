...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> findNearestNeighbor_help(const Point<Dim>& query, KDTreeNode *root, int Dimension) const;

    // Helper function DestroyTree
    void DestroyTree(KDTreeNode *curRoot);

    // Helper function BuildTree_copy
    KDTreeNode* BuildTree_copy(KDTreeNode *curRoot);
    ...
    Point<Dim> Quickselect(vector<Point<Dim>>& list, int left, int right, int k, int Dimension);

    // Helper function Partition
    int Partition(vector<Point<Dim>>& list, int left, int right, int pivot_index, int Dimension);
};
