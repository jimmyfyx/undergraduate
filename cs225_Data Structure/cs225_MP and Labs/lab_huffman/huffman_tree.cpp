                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty() == true && mergeQueue.empty() == true)
    {
        return NULL;
    }

    // One of the queue is empty
    if (singleQueue.empty() == true && mergeQueue.empty() == false)
    {
        TreeNode *node = mergeQueue.front();
        mergeQueue.pop();
        return node;
    }
    else if (singleQueue.empty() == false && mergeQueue.empty() == true)
    {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // Push all the TreeNode pointers into the queue
    ...
    while ( ((singleQueue.empty() && mergeQueue.size() == 1) || (mergeQueue.empty() && singleQueue.size() == 1)) == false )
    {
        // Remove the two smallest nodes from the queues
        TreeNode *node_1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode *node_2 = removeSmallest(singleQueue, mergeQueue);
        // Create and initialize the new internal node
        TreeNode *parent = new TreeNode((node_1 -> freq).getFrequency() + (node_2 -> freq).getFrequency());
        parent -> left = node_1;
        parent -> right = node_2;
        // Push the new node into the mergeQueue
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (current -> left != NULL && next == false)
        {
            current = current -> left;
        }
        else if (current -> right != NULL && next == true)
        {
