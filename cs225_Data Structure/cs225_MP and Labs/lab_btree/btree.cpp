...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    else
    {
        ...
        {
            // The node is a leaf node and we cannot find the value
            return V();
        }
        else
        {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if (first_larger_idx >= 0 && first_larger_idx < (subroot->elements).size() && (subroot->elements)[first_larger_idx] == pair)
    {
        // The value already exists, no need to insert
        return; 
    }
    else
    {
        ...
        if (subroot -> is_leaf == true)
        {
            // The current node is a leaf node, simply insert the pair into the elements vector
