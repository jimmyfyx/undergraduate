...
/* Helper function binay insert on a sorted vector */
template <class T, class C>
int binary_insert(const std::vector<T>& elements, unsigned int left, unsigned int right, const C& val)
{
    /*
    ...
    if (val > elements[mid])
    {
        return binary_insert(elements, mid + 1, right, val);
    }

    return binary_insert(elements, left, mid - 1, val);
    */
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return binary_insert(elements, 0, size - 1, val);
}
