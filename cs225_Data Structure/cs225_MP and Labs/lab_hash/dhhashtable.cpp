...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    size_t hash_1 = hashes::hash(key, size);
    size_t hash_2 = hashes::secondary_hash(key, size);
    for (size_t i = 0; i < size; i ++)
    {
        size_t index = (hash_1 + hash_2*i) % size;
        if (table[index] == NULL)
        {
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    {
        size_t index = (hash_1 + hash_2*i) % size;
        if (should_probe[index] == true && table[index] != NULL && table[index] -> first == key)
        {
            return index;
        }
    }
