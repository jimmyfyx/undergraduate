...
class ScoreMatching():
    ...
    def langevin_dynamics_sample(self, x_mod, noise_levels, iterations, lr):
        ...
        epsilon = lr
        for i in range(0, L):
            x_prev = torch.clone(x_mod)
            alpha = epsilon * ((noise_levels[i]) ** 2 / (noise_levels[L - 1]) ** 2)
            for t in range(0, T):
                z = torch.normal(mean=torch.zeros(2000, 2), std=torch.ones(2000, 2))
                sigma_batch = torch.ones(2000, 2) * noise_levels[i]
                x_prev = x_prev + (alpha / 2) * self.scorenet(x_prev, sigma_batch) + torch.sqrt(alpha) * z
            x_mod = x_prev
    ...
    def denoising_loss(self, samples, noise_levels):
        ...
        sigma_chosen_1 = torch.cat((torch.reshape(sigma_chosen, (2000, 1)), torch.reshape(sigma_chosen, (2000, 1))), dim=1)
        sigma_chosen_1_sq = torch.pow(sigma_chosen_1, 2)

        #TODO: YOUR CODE HERE
        # sample x_tilde
        x_tilde = torch.normal(samples, sigma_chosen_1)

        loss = self.scorenet(x_tilde, sigma_chosen_1)
        loss = loss + (x_tilde - samples) / sigma_chosen_1_sq
