...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # first find the position of the number 0
        zero_pos = (0, 0)
        ...
        # check below
        if zero_pos[0] >= 0 and zero_pos[0] < 2:
            copy_state = copy.deepcopy(self.state)
            copy_state[zero_pos[0]][zero_pos[1]] = copy_state[zero_pos[0] + 1][zero_pos[1]]
            copy_state[zero_pos[0] + 1][zero_pos[1]] = 0
            below_state =  EightPuzzleState(copy_state, self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
            nbr_states.append(below_state)
        
        
        # check left
        if zero_pos[1] > 0 and zero_pos[1] <= 2:
            copy_state = copy.deepcopy(self.state)
            copy_state[zero_pos[0]][zero_pos[1]] = copy_state[zero_pos[0]][zero_pos[1] - 1]
            copy_state[zero_pos[0]][zero_pos[1] - 1] = 0
            left_state =  EightPuzzleState(copy_state, self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
        ...
        if zero_pos[0] > 0 and zero_pos[0] <= 2:
            ...
            copy_state[zero_pos[0]][zero_pos[1]] = copy_state[zero_pos[0] - 1][zero_pos[1]]
            copy_state[zero_pos[0] - 1][zero_pos[1]] = 0
            above_state =  EightPuzzleState(copy_state, self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
        ...
        if zero_pos[1] >= 0 and zero_pos[1] < 2:
            ...
            copy_state[zero_pos[0]][zero_pos[1]] = copy_state[zero_pos[0]][zero_pos[1] + 1]
            copy_state[zero_pos[0]][zero_pos[1] + 1] = 0
            right_state =  EightPuzzleState(copy_state, self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
    ...
    def compute_heuristic(self):
        ...
        # for each number calculate the heuristic dist and sum up
        for i in range(len(self.state[0])):
            for j in range(len(self.state[1])):
                cur_pos = (i, j)
                if self.state[i][j] != 0:
                    # print("{num} heuristic: {heu}".format(num=self.state[i][j], heu=manhattan(cur_pos, goal_pos[self.state[i][j]])))
                    total += manhattan(cur_pos, goal_pos[self.state[i][j]])
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for loc in neighboring_grid_locs:
            grid_obj = SingleGoalGridState(loc, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors)
            nbr_states.append(grid_obj)
        
        return nbr_states

    # TODO(V): implement this method, check if the current state is the goal state
    def is_goal(self):
        return self.state[0] == self.goal[0][0] and self.state[1] == self.goal[0][1]
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for loc in neighboring_locs:
            # check if the neighbor is one of the goal state
            if loc in self.goal:
                # remove the goal state
                goal = copy.deepcopy(self.goal)
                goal_list = list(goal)
                goal_list.remove((loc[0], loc[1]))
                goal_tuple = tuple(goal_list)
                grid_obj = GridState(loc, goal_tuple, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
                nbr_states.append(grid_obj)
            else:
                grid_obj = GridState(loc, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
                nbr_states.append(grid_obj)
    ...
    def compute_heuristic(self):
        # obtain the value of MST cost for self.goal
        mst_cost = 0
        if (len(self.goal) != 0):
        ...
        # obtain the manhattan distance to the nearest goal
        min_manh_dist = 0
        if (len(self.goal) != 0):
            min_manh_dist = float('inf')
            for goal in self.goal:
                cur_dist = manhattan(self.state, goal)
                if cur_dist < min_manh_dist:
                    min_manh_dist = cur_dist
        
        return mst_cost + min_manh_dist
