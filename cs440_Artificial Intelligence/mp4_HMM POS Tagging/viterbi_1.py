...
def viterbi_1(train, test):
    ...
    emission_dict = {}
    transition_dict = {}
    # another dict to store all possible tags for each word
    word_tag = {}
    ...
    for tag in emission_dict:
        ...
        for word in emission_dict[tag]:
            emission_dict[tag][word] = (emission_dict[tag][word] + laplace_em) / (n + laplace_em * (V + 1))
            emission_dict[tag][word] = math.log(emission_dict[tag][word])
        # assign probability for unknown words
        emission_dict[tag]['UNK'] = math.log(laplace_em / (n + laplace_em * (V + 1)))
