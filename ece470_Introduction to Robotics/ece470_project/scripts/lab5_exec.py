...
target = [
    [0.35,0,block_height],
    [0.35,0.07,block_height],
    [0.35, 0.14, block_height],
    [0.35,0.21, block_height]
]
...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    move_arm(pub_cmd, loop_rate, lab_invk(start_xw_yw_zw[0], start_xw_yw_zw[1], 0.15, 0), 4.0, 4.0)

    # Move to start location and check for block
    ...
    # If there is a block, move to end location via higher locations to avoid collisions
    move_arm(pub_cmd, loop_rate, lab_invk(start_xw_yw_zw[0], start_xw_yw_zw[1], 0.15, 0), 4.0, 4.0)
    move_arm(pub_cmd, loop_rate, lab_invk(target_xw_yw_zw[0], target_xw_yw_zw[1], 0.15,0), 4.0, 4.0)
...
class ImageConverter:
    ...
    def image_callback(self, data):
        ...
        xw_yw_G = blob_search(cv_image, "lavender")
...
def main():
    ...
    xw_yw_G_fixed = copy.deepcopy(xw_yw_G)
    xw_yw_Y_fixed = copy.deepcopy(xw_yw_Y)
    print(xw_yw_G_fixed)
    print(xw_yw_Y_fixed)
    error = 0
    for count in range(4):
        ...
        if(count == 1 or count == 0):
            error = move_block(pub_command, loop_rate, [xw_yw_Y_fixed[count%2][0],xw_yw_Y_fixed[count%2][1], block_height], target[count], 4, 4)

        elif(count == 2 or count == 3):
            error = move_block(pub_command, loop_rate, [xw_yw_G_fixed[count%2][0],xw_yw_G_fixed[count%2][1], block_height], target[count], 4, 4)
